  ...
  const renderNavItem = (item: (typeof PRIMARY_NAVIGATION)[number]) => {
    ...
    return (
      ...
      >
        <span
          className={cn(
            "flex min-h-11 min-w-11 max-w-full flex-col items-center justify-center gap-0.5 px-1 text-[11px] font-semibold leading-none text-muted-foreground transition-[color,transform] duration-200 ease-out group-hover:text-foreground group-active:scale-95",
            active && "text-primary group-hover:text-primary"
          )}
        >
          <span
            className={cn(
              "flex h-7 min-w-10 items-center justify-center rounded-lg px-2 transition-[background-color,transform] duration-200 ease-out",
              active ? "bg-primary/10" : "group-hover:bg-muted/60"
            )}
          >
            <Icon
              className={cn(
